...
如果 MCP 服务器的搜索结果包含 `candidates` 和 `capabilityToken`，可用原始搜索与调用名称配置 `capabilityBroker`。模型最初只看到搜索工具，看不到调用工具。成功且非空的搜索会从模型可见内容中移除令牌，将其保存在该 Agent 作用域内，并发布仅限返回候选项的调用 schema；合法的零候选结果会正常成功，并移除之前的作用域调用工具。调用 schema 要求 `arguments` 和可选的 `references` 为对象且不包含 `capabilityToken`；桥接层注入可信令牌。如果模型仍把任一对象发送为包含单个 JSON 对象的字符串，桥接层会进行一次兼容解码。无效 JSON、数组、标量、未知字段以及不属于最新搜索结果的工具 id 会在 MCP 调用前失败。
